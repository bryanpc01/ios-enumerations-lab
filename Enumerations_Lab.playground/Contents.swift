...
// */
//
//// Define an enumeration called `iOSDeviceTypes` with member values `iPhone`, `iPad`, `iWatch`.
...
//print("A pentagon has \(pentagon.numberOfSides()) number of sides.")
//
//// Re-write `Shape` so that each case has an associated value of type Int that will represent the length of the sides (assume the shapes are regular polygons so all the sides are the same length) and write a method inside that returns the perimeter of the shape.
//print("The equalatiral pentagon has a perimeter of \(pentagon.perimeter())")
